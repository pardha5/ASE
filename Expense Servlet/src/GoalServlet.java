...
public class GoalServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		BasicDBObject goal1 = new BasicDBObject(params);
		
		for(Object key : params.keySet().toArray()) {
			goal1.put(key.toString(), params.get(key));
		}
		
		System.out.println(goal1.toJson());
		
		MongoClientURI uri = new MongoClientURI("mongodb://root:root@ds037234.mongolab.com:37234/expensetracker");
		MongoClient client = new MongoClient(uri);

		DB db = client.getDB(uri.getDatabase());
		DBCollection goal = db.getCollection("goal");
		WriteResult result = goal.insert(goal1);
